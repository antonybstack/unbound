...
| **3 — a character that persists** | Playable | `character` row keeps XP/name/loadout across disconnect. Client stores the STDB access token so a relaunch is the same wanderer with the same bag. Skills go up when you land hits (level-up toast). HUD shows XP into the current level. Sheathed `E` gathers wood/ore into Gathering; walking into E-range pulses the gather line. |
