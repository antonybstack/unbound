...
**MVP (slices 0–3) is playable.** Predicted WASD and swing start, sheathe/draw camera, three loadouts, a dummy with a readable windup, 1v1 PvP, and a `character` row that keeps XP after you leave the yard. Native identity lives in `~/.local/share/unbound/identity.token`; the browser uses `localStorage`.
...
./scripts/web.sh                    # http://127.0.0.1:8080 (debug wasm)
./scripts/web-release.sh            # smaller WebGPU bundle in crates/client/dist
```
